...
/**
 * Redis DAO for {@link AxeSession}.
 *
 * @since 3.8
 */
@RequiredArgsConstructor
@Repository
public class AxeSessionRedisDao extends RedisDao {
    private static final int LOCK_TTL_SECONDS = 7;

    private final RedisTemplate<String, AxeSession> redisTemplate;
    ...
    private ValueOperations<String, AxeSession> valueOps;
    ...
    /**
     * Checks if {@link AxeSession} with given ID exists in Redis or not.
     *
     * @param sessionId non-empty string with session id to check.
     * @return true if {@link AxeSession} found, false - if not.
    ...
    /**
     * Retrieve {@link AxeSession} by its ID.
     *
     * @param sessionId string with session id used as key.
     * @return {@link Optional} which contains {@link AxeSession} or not.
     * @throws IllegalArgumentException when session io is empty or null.
     */
    public Optional<AxeSession> get(final String sessionId) {
    ...
    /**
     * Updates {@link AxeSession} and preserves TTL.
     *
     * @param session object to store.
     * @throws IllegalArgumentException when session is null.
     * @see #getRecordTtl()
     */
    public void update(final AxeSession session) {
