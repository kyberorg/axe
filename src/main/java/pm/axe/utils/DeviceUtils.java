...
public final class DeviceUtils {
    ...
    /**
     * Defines if current device is Mobile or not, based on User-Agent stored in {@link AxeSession}.
     *
     * @return true - if device is mobile phone, false if not.
     */
    public static boolean isMobileDevice() {
        if (AxeSession.getCurrent().isPresent()) {
            AxeSession session = AxeSession.getCurrent().get();
