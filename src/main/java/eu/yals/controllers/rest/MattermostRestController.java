...
public class MattermostRestController {
    ...
    private MattermostResponseJson success(Link savedLink) {
        ...
        if (StringUtils.isBlank(linkDescription)) {
            String userGreet = StringUtils.isNotBlank(mattermost.getUsername()) && (!mattermost.getUsername().equals(Mattermost.Constants.NO_VALUE)) ?
                    "Okay " + Mattermost.Constants.AT + mattermost.getUsername() + ", " : "Okay, ";
    ...
    private MattermostResponseJson usage() {
        ...
        return MattermostResponseJson.createWithText(Mattermost.Emoji.INFO + "  Usage: " + command +
                " http://mysuperlonglink.tld [Optional Link Description]");
    }

    private MattermostResponseJson serverError() {
        return MattermostResponseJson.createWithText(Mattermost.Emoji.WARNING + " Server Error")
